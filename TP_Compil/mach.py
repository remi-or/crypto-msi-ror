#!/usr/bin/env python3
#
# $Id: mach.py 552 2020-09-17 09:12:44Z coelho $
#
...
is_onearg = re.compile('^(' + ONEARG + ')$').match
is_noarg = re.compile('^(' + NOARG + ')$').match
is_integer = re.compile('^-?\d+$').match
is_empty = re.compile('^[ \t\r]+\n$').match
split = re.compile('[ \n\t\r]+')
...
class Machine:
    ...
    def check_code(self):
        ...
        for line in open(sys.argv[1], 'r').readlines():
            ...
            if len(cmds) == 0 or (len(cmds) == 1 and cmds[0] == ''):
                continue
            elif is_onearg(cmds[0]):
                assert len(cmds) == 2
                if not is_integer(cmds[1]):
                    self.err('expecting integer, got %s on line %d' %
                             (cmds[1], nline))
            elif is_noarg(cmds[0]):
                assert len(cmds) == 1
            else:
    ...
    EXEC = {
        ...
        'NOT': _not,
        'OUT': out, 'IN': _in,
        'GOTO': goto, 'BGZ': bgz, 'BEZ': bez
    }

    def __str__(self):
        return "#pc=%d [%s]\n#stack: %s\n#data: %s\n" % \
                (self.pc, n2i[self.program[self.pc]], self.stack, self.data)
